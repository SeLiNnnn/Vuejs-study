    ...
    <!--footNav-->
      <footer class="ft">
          ...
          <router-link class="link"  :to="{name:'shopcart'}">
              <div class="svg iconfont icon-gouwuche count"><i class="showNum" v-if="this.picNum">8</i></div>
              <p>购物</p>
          </router-link>
...
export default {
  ...
  created() {
    ...
    this.$ajax
      .get(this.dataURL("vue.php", "vueTitle"))
      .then(res => {
...
.ft {
  ...
  .fade-enter,
  .fade-leave-to,
  .fade-leave-active below version {
    opacity: 0;
  }
}
</style>